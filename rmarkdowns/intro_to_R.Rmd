...
```{r, message=FALSE}
sdr_data <- read.csv(here("data/SDR-2023-Data.csv")) # assigning the dataset to a variable so that we can look at/reference it
...
#challenge 2
sdr_data %>%
  select(goal_5_score, goal_7_score)
```
...
sdr_data %>% 
  ...
  summarise(mean_goal_7_score_by_region = mean(population_using_at_least_basic_drinking_water_services))
```
...
```{r}
sdr_data %>% 
  select(country, population_with_access_to_clean_fuels_and_technology_for_cooking, co_emissions_from_fuel_combustion_per_total_electricity_output_mt_co_t_wh, population_with_access_to_electricity, renewable_energy_share_in_total_final_energy_consumption) %>%
